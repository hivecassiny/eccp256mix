...
SERVICE_NAME="eccp256mix"
INSTALL_DIR="/opt/eccp256mix"
BIN_PATH="$INSTALL_DIR/eccp256mix"
SERVICE_FILE="/etc/systemd/system/$SERVICE_NAME.service"
DOWNLOAD_URL=$1
TEMP_DIR="/tmp/eccp256mix_install"
LOG_FILE="/var/log/eccp256mix_install.log"
...
install_program() {
    ...
    # 下载文件
    local download_path="$TEMP_DIR/eccp256mix.tar.gz"
    ...
    if [[ ! -f "$BIN_PATH" ]]; then
        # 可能在解压后的子目录中
        find "$INSTALL_DIR" -name "eccp256mix" -type f -exec mv {} "$INSTALL_DIR/" \;
        
        if [[ ! -f "$BIN_PATH" ]]; then
            print_color "未找到可执行文件 eccp256mix" "$RED"
...
show_menu() {
    clear
    print_color "========================================" "$BLUE"
    print_color "    eccp256mix "$APPVERSION" 安装管理程序" "$GREEN" 
    print_color      "$CHANGEFILEINFO"
    print_color "========================================" "$BLUE"
    echo
    print_color "请选择操作:" "$BLUE"
    echo
    print_color "  1. 安装 eccp256mix" "$YELLOW"
    print_color "  2. 卸载 eccp256mix" "$RED"
    print_color "  3. 重新安装 eccp256mix" "$YELLOW"
